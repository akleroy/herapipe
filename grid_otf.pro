...
;             ... ELSE JUST GET THE AVERAGE SPECTRUM
                 ...
                 if nchan eq 1 then begin
                    spectrum = total(data[keep] * combined_weight)
                 endif else begin
...
; &%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%
; APPLY APODIZATION IF REQUESTED (REALLY NOT READY TO GO, DON'T USE THIS!)
; &%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%

; ... I'M TORN ON THE FFT HERE AT THE EDGE, WE WANT IT TO GO TO
; ZERO. FILLING IN THE INTERIOR OF THE MAP, NOT REALLY.
  if keyword_set(apodize) then begin
     
     apod_fwhm_pix = apod_fwhm / abs(sxpar(target_hdr,'CDELT1'))
     apod_kern_pix = (apod_fwhm_pix)*4.0 + 1
     apod_kern = psf_gaussian(npix = apod_kern_pix, fwhm = apod_fwhm_pix, /norm)
     
     for i = 0, nchan-1 do begin
        counter, i+1, nchan, 'Apodizing plane ...'
                
        finite_map = float(finite(data_cube[*,*,i]))
        if total(finite_map) eq 0 then continue
        plane = data_cube[*,*,i]
        
        response = convol(finite_map, apod_kern, /edge_truncate)
        
        replace = where(response gt 0.25 and finite_map eq 0, replace_ct)

        if replace_ct eq 0 then continue
        
        smooth_plane = convol(plane, apod_kern, /nan, /edge_truncate)       
        plane[replace] = smooth_plane[replace]
