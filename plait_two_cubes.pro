...
; &%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%
  ...
  if angle_diff ne 90 then begin
     message, 'Maps not 90 degrees out of phase. Stopping.'    
  endif
...
; THIS NEEDS IMPROVEMENT. FOR THE MOMENT, IT LOOKS TO ME LIKE THE FIRST 10 OR
; SO PIXELS IN NGC3184/NGC3351 THE FFT CONTAIN MOST OF THE EXCESS POWER IN THE
; SCAN DIRECTION.
...
; &%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%
  ...
  if keyword_set(test_accuracy) and abs(rot) gt 1. then begin
     ...
     message, 'Rotation introduces '+string(mad((cube_hori/orig_cube_hori)[ind]))+ $
              ' scatter about '+string(median((cube_hori/orig_cube_hori)[ind])), /info
     message, 'Rotation introduces '+string(mad((cube_vert/orig_cube_vert)[ind]))+ $
              ' scatter about '+string(median((cube_vert/orig_cube_vert)[ind])), /info
  endif
